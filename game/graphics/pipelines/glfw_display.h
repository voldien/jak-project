...
class GLFWDisplay : public GfxDisplay {
 public:
  GLFWDisplay(GLFWwindow* window, bool is_main);
  virtual ~GLFWDisplay();
  ...
  virtual void on_window_pos(GLFWwindow* window, int xpos, int ypos);
  virtual void on_window_size(GLFWwindow* window, int width, int height);
  virtual void on_iconify(GLFWwindow* window, int iconified);
  virtual void on_mouse_key(GLFWwindow* window, int button, int action, int mode);
  virtual void on_cursor_position(GLFWwindow* window, double xposition, double yposition);
  virtual void update_cursor_visibility(GLFWwindow* window, bool is_visible);

 protected:
  virtual void update_glfw();
 ...
 public:
  static void SetGlobalGLFWCallbacks();
  static void ClearGlobalGLFWCallbacks();

  GLFWmonitor* get_monitor(int index);
};

// TODO relocate
typedef struct glfw_state {
  bool callbacks_registered = false;
  GLFWmonitor** monitors;
  int monitor_count;
} GlFWState;

static GlFWState g_glfw_state;
